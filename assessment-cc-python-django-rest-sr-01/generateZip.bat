...
for /f "tokens=*" %%f in ('git diff --name-only HEAD') do (
    ...
    if !match! equ true (
        REM Allow changes in migrations folders
        if "!file:~0,18!"=="battle/migrations/" (
            REM File is in battle migrations, allow change
            REM Do nothing
        ) else if "!file:~0,19!"=="monster/migrations/" (
            REM File is in monster migrations, allow change
            REM Do nothing
        ) else (
