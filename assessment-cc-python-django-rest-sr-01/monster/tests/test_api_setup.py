...
class MonsterAPISetUp(APITestCase):
    ...
    def setUp(self):
        ...
        with open(file_path("monsters-empty-monster.csv"), "rb") as empty_data:
            self.empty_csv = SimpleUploadedFile(
                name="monsters-empty-monster.csv",
                content=empty_data.read(),
                content_type="multipart/form-data",
            )

        with open(file_path("monsters-wrong-column.csv"), "rb") as wrong_data:
            self.wrong_csv = SimpleUploadedFile(
                name="monsters-wrong-column.csv",
                content=wrong_data.read(),
                content_type="multipart/form-data",
            )
